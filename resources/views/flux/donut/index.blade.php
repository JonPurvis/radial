...
@php
    ...
    $viewBoxSize = ($radius + $hoverThickness) * 2;
...
>
    @if ($showLegend && $legendPosition === 'left')
        <div class="flex flex-col gap-1 pr-4 shrink-0 order-first">
            @foreach ($segments as $i => $segment)
                <flux:donut.legend :index="$i" :class="$segment['class']" :label="$segment['label']" />
            @endforeach
        </div>
    ...
    <div
        class="flex justify-center items-center w-full {{ $showLegend ? 'shrink-0' : 'h-full' }}"
        x-ref="container"
    >
        <svg
        ...
        >
            @foreach ($segments as $i => $segment)
                <circle
                    ...
                    :class="hovered === {{ $i }} ? 'opacity-100' : (hovered !== null ? 'opacity-30' : 'opacity-100')"
        ...
        @if ($tooltip)
            ...
            >
                <template x-if="hovered !== null">
                    <div>
                        <div class="flex items-center justify-between p-2 text-xs font-medium text-zinc-800 bg-zinc-50 border-b border-zinc-200 dark:text-zinc-100 dark:bg-zinc-600 dark:border-zinc-500">
                        ...
                        <div class="flex items-center gap-2 p-2 text-xs text-zinc-500 dark:text-zinc-300">
                            ...
                            <span class="text-zinc-800 dark:text-white">Value</span>
    ...
    @if ($showLegend && $legendPosition === 'right')
        <div class="flex flex-col gap-1 pl-4 shrink-0">
            @foreach ($segments as $i => $segment)
                <flux:donut.legend :index="$i" :class="$segment['class']" :label="$segment['label']" />
            @endforeach
        </div>
